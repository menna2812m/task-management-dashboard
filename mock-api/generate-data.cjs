...
function main() {
  ...
  try {
    ...
    const statisticsResponse = generateStatistics();
    const tasksResponse = generateTasks();

    const users = [
      ...new Map(tasksResponse.tasks.map((task) => [task.assignee.id, task.assignee])).values(),
    ];
